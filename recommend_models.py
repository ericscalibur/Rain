...
def recommend(profile: HardwareProfile) -> List[Recommendation]:
    ...
    if profile.is_apple_silicon:
        gen   = profile.apple_chip_gen
        tier  = profile.apple_chip_tier
        umem  = profile.unified_memory_gb

        if umem >= 128:
            add("llama3.3:70b",        "must",    "128 GB unified — 70B runs comfortably.")
            add("deepseek-r1:70b",     "strong",  "Full 70B reasoning chain-of-thought.")
            add("qwen2.5:72b",         "strong",  "Multilingual 72B flagship.")
            add("qwen2.5-coder:32b",   "must",    "Repo-scale code model.")
            add("deepseek-r1:32b",     "strong",  "32B reasoning — fast at this pool size.")
            add("qwen3.5:9b",          "strong",  "Keep for fast routing / reflection.")
            add("llama3.2-vision:11b", "strong",  "Vision pipeline.")
            add("codestral:latest",    "strong",  "Best code FIM model.")
        elif umem >= 64:
            add("deepseek-r1:32b",     "must",    "64 GB unified — 32B runs well.")
            add("qwen2.5-coder:32b",   "must",    "Best open code model at this tier.")
            add("llama3.3:70b",        "optional","70B fits but is tight — slow on base M-chips.")
            add("qwen3.5:9b",          "must",    "Fast agent for routing and reflection.")
            add("llama3.2-vision:11b", "strong",  "Vision pipeline.")
            add("codestral:latest",    "strong",  "Code FIM model.")
            add("qwen3:8b",            "strong",  "Sharp 8B. Strong at summarization and rewriting.")
        elif umem >= 32:
            add("qwen3.5:9b",          "must",    "Best mid-size reasoning model. Strong at logic and analysis.")
            add("qwen2.5-coder:14b",   "must",    "14B code model fits well at 32 GB.")
            add("qwen3:8b",            "must",    "Sharp 8B. Strong at summarization and rewriting.")
            add("llama3.2:latest",     "must",    "Fast reflection / small tasks.")
            add("llama3.2-vision:11b", "strong",  "Vision pipeline (11B fits fine).")
            add("codestral:latest",    "strong",  "12B code model — fits with headroom.")
            add("deepseek-r1:8b",      "optional","Reasoning model with CoT.")
            add("deepseek-r1:32b",     "upgrade", "32B reasoning — tight but possible.")
        elif umem >= 16:
            add("qwen3.5:9b",          "must",    "Core reasoning agent — fits at 16 GB.")
            add("qwen2.5-coder:7b",    "must",    "Best 7B code model. Great for generation, debugging, and refactoring.")
            add("qwen3:8b",            "must",    "Sharp 8B. Strong at summarization and rewriting.")
            add("llama3.2:latest",     "must",    "Fast reflection agent.")
            add("llama3.1:8b",         "strong",  "Strong general 8B.")
            add("deepseek-r1:8b",      "optional","Reasoning with CoT — fits at 16 GB.")
            add("llama3.2-vision:11b", "optional","Vision — tight but usually works.")
            add("codestral:latest",    "upgrade", "12B — too large unless only model loaded.")
            add("qwen2.5-coder:14b",   "upgrade", "14B — over budget at 16 GB.")
        elif umem >= 8:
            add("llama3.2:latest",     "must",    "Small but capable — fits in 8 GB.")
            add("qwen2.5-coder:7b",    "must",    "7B code — fits but is snug.")
            add("qwen3:8b",            "strong",  "8B synthesizer — fits if nothing else loaded.")
            add("phi3:mini",           "strong",  "3.8B — fast and capable for 8 GB.")
            add("qwen2.5:1.5b",        "optional","Tiny fallback for instant responses.")
            add("qwen3.5:9b",          "upgrade", "9B — just over budget, upgrade RAM first.")
            add("llama3.1:8b",         "optional","8B — snug at 8 GB unified.")
        else:
            add("llama3.2:3b",         "must",    "3B is your best bet under 8 GB.")
            add("phi3:mini",           "strong",  "3.8B — strong reasoning per GB.")
            add("qwen2.5:1.5b",        "strong",  "1.5B for fast completions.")
            add("qwen2.5:0.5b",        "optional","Tiny emergency fallback.")
    ...
    # ── NVIDIA CUDA ───────────────────────────────────────────────────────
        ...
        if vram >= 80:
            ...
            add("deepseek-r1:70b",     "strong",  "Full reasoning 70B.")
            add("qwen2.5-coder:32b",   "must",    "32B code model.")
        elif vram >= 48:
            add("deepseek-r1:32b",     "must",    f"{vram:.0f} GB VRAM — 32B fits well.")
            add("qwen2.5-coder:32b",   "must",    "32B code model.")
            add("llama3.3:70b",        "optional","70B — tight but fits with Q4.")
        elif vram >= 24:
            add("deepseek-r1:32b",     "strong",  f"{vram:.0f} GB VRAM — 32B at Q4.")
            add("qwen2.5-coder:14b",   "must",    "14B code — runs well.")
            add("qwen3.5:9b",          "must",    "Primary reasoning agent.")
            add("codestral:latest",    "strong",  "12B code FIM model.")
            add("llama3.2-vision:11b", "strong",  "Vision pipeline.")
            add("qwen3:8b",            "must",    "Sharp 8B. Strong at summarization and rewriting.")
            add("llama3.2:latest",     "must",    "Fast reflection agent.")
        elif vram >= 16:
            add("qwen2.5-coder:14b",   "strong",  f"{vram:.0f} GB VRAM — 14B code fits.")
            add("qwen3.5:9b",          "must",    "Primary reasoning agent.")
            add("qwen3:8b",            "must",    "Sharp 8B. Strong at summarization and rewriting.")
            add("llama3.2:latest",     "must",    "Fast reflection agent.")
            add("qwen2.5-coder:7b",    "must",    "DEV agent — always pull this.")
            add("llama3.2-vision:11b", "optional","Vision — fits at 16 GB.")
            add("deepseek-r1:8b",      "optional","Reasoning with CoT.")
        elif vram >= 10:
            add("qwen2.5-coder:7b",    "must",    f"{vram:.0f} GB VRAM — 7B code fits well.")
            add("qwen3.5:9b",          "must",    "Primary agent — fits at this VRAM.")
            add("qwen3:8b",            "strong",  "Synthesizer — snug but fits.")
            add("llama3.2:latest",     "must",    "Reflection agent.")
            add("mistral:7b",          "optional","Strong instruction following 7B.")
        elif vram >= 6:
            add("qwen2.5-coder:7b",    "must",    f"{vram:.0f} GB VRAM — 7B at Q4 fits.")
            add("llama3.2:latest",     "must",    "Reflection agent.")
            add("phi3:mini",           "strong",  "3.8B — fast and capable.")
    ...
    # ── CPU-only (no GPU / no Apple Silicon) ─────────────────────────────
        ...
        ram = profile.ram_gb
        if ram >= 128:
            add("llama3.3:70b",        "strong",  f"{ram:.0f} GB RAM — 70B possible but slow on CPU.")
            add("deepseek-r1:32b",     "must",    "32B reasoning model — usable on high-RAM CPU.")
            add("qwen2.5-coder:14b",   "must",    "14B code — reasonable CPU speed.")
            add("qwen3.5:9b",          "must",    "Primary reasoning agent.")
            add("qwen3:8b",            "must",    "Synthesizer.")
            add("llama3.2:latest",     "must",    "Reflection agent.")
        elif ram >= 64:
            add("deepseek-r1:32b",     "optional","32B — slow on CPU but possible.")
            add("qwen2.5-coder:14b",   "strong",  f"{ram:.0f} GB RAM — 14B fits.")
            add("qwen3.5:9b",          "must",    "Primary reasoning agent.")
            add("qwen3:8b",            "must",    "Synthesizer.")
            add("llama3.2:latest",     "must",    "Reflection agent.")
            add("qwen2.5-coder:7b",    "must",    "DEV agent.")
        elif ram >= 32:
            add("qwen3.5:9b",          "must",    f"{ram:.0f} GB RAM — 9B usable on CPU.")
            add("qwen2.5-coder:7b",    "must",    "DEV agent.")
            add("qwen3:8b",            "must",    "Synthesizer.")
            add("llama3.2:latest",     "must",    "Reflection agent.")
            add("mistral:7b",          "optional","7B alternative.")
        elif ram >= 16:
            add("qwen2.5-coder:7b",    "strong",  f"{ram:.0f} GB RAM — 7B is the ceiling.")
            add("llama3.2:latest",     "must",    "Reflection agent — small and fast.")
            add("phi3:mini",           "must",    "3.8B — best quality for CPU-only.")
            add("qwen3.5:9b",          "upgrade", "9B — needs more RAM for comfortable CPU use.")
        else:
            add("phi3:mini",           "must",    f"{ram:.0f} GB RAM — 3.8B is the ceiling.")
            add("llama3.2:3b",         "must",    "3B fast model.")
            add("qwen2.5:1.5b",        "strong",  "1.5B for instant responses.")
